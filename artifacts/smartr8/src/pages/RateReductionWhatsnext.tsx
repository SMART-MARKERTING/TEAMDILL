...
import { ArrowRight, BarChart2, CalendarDays, FileText } from "lucide-react";
...
export default function RateReductionWhatsnext() {
  const search = useSearch();
  const firstName = new URLSearchParams(search).get("name") || "";
  const ga4 = useGA4("rate-reduction");

  useEffect(() => {
    trackFbEvent("Lead", { content_name: "Rate Reduction", content_category: "Mortgage" });
  }, []);
            ...
            <Card className="shadow-sm border-primary/20 flex flex-col">
              <CardContent className="p-6 flex flex-col gap-4 h-full">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">Rate reduction is highly personal. Let's talk through your options together to make sure we get you the best fit.</p>
                </div>
                <div className="mt-auto">
                  <a href={CAL_URL} target="_blank" rel="noopener noreferrer" onClick={() => ga4.trackWhatsnextClick("schedule_call")}>
                    <Button className="w-full h-11 bg-accent hover:bg-accent/90 text-white">Book a Time <ArrowRight className="ml-2 h-4 w-4" /></Button>
                  </a>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-sm flex flex-col">
              <CardContent className="p-6 flex flex-col gap-4 h-full">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">Ready to start your refinance application?</p>
                </div>
                <div className="mt-auto">
