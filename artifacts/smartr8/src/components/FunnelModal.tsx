...
export function FunnelModal({ isOpen, onClose, initialGoal }: FunnelModalProps) {
  ...
  // Update initialGoal if modal opens with it
  React.useEffect(() => {
    if (isOpen) {
      if (initialGoal) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || "There was a problem submitting your form.");
      }
    } catch (err) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md w-full h-[100dvh] sm:h-auto sm:rounded-xl p-0 flex flex-col gap-0 border-0 bg-white">
        <DialogTitle className="sr-only">See your options</DialogTitle>
        <DialogDescription className="sr-only">Answer a few questions to see your mortgage options</DialogDescription>
        
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b">
          {step > 1 ? (
            <Button variant="ghost" size="icon" onClick={handleBack} className="h-8 w-8 text-muted-foreground">
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8">
          <form id="funnel-form" onSubmit={handleSubmit} className="h-full flex flex-col justify-center">
            ...
            {step === 6 && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col h-full">
                ...
                <div className="flex-1 flex flex-col gap-4">
                  ...
                  <div className="flex items-start space-x-3 mt-4 bg-secondary/50 p-4 rounded-lg">
                    <Checkbox 
                      id="consent" 
