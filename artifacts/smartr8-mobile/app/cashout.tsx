...
import { ChoiceCard, FunnelButton, FunnelInput, FunnelScreen, MultiChoiceCard, StepHeading } from "@/components/FunnelScreen";
...
const CASH_RANGES = ["Under $50,000","$50,000 – $100,000","$100,000 – $250,000","$250,000 – $500,000","Over $500,000"];
...
type FS = {
  ...
  homeValue: string; mortgageBalance: string; cashNeeded: string;
  cashPurposes: string[]; creditScore: string;
  dobMonth: string; dobDay: string; dobYear: string;
  email: string; phone: string;
};
const DEFAULT: FS = { step:1, firstName:"", lastName:"", address:"", city:"", stateCode:"", zip:"", homeValue:"", mortgageBalance:"", cashNeeded:"", cashPurposes:[], creditScore:"", dobMonth:"", dobDay:"", dobYear:"", email:"", phone:"" };
...
export default function CashOutFunnel() {
  ...
  return (
    <FunnelScreen step={st.step} totalSteps={TOTAL} onBack={back} label="CASH-OUT REFI">
      {st.step === 1 && (
        <>
      ...
      {st.step === 2 && (
        <>
          ...
          <View style={styles.row}>
            <View style={styles.stateField}><FunnelInput label="State" value={st.stateCode} onChangeText={v => p({ stateCode:v.toUpperCase() })} placeholder="CA" maxLength={2} autoCapitalize="characters" testID="input-state" /></View>
            <View style={styles.zipField}><FunnelInput label="ZIP" value={st.zip} onChangeText={v => p({ zip:v })} placeholder="92660" keyboardType="numeric" maxLength={5} testID="input-zip" /></View>
          </View>
      ...
      {st.step === 3 && (
        <>
          ...
          {HOME_VALUE_RANGES.map(opt => <ChoiceCard key={opt} label={opt} selected={st.homeValue === opt} onPress={() => autoAdvance({ homeValue:opt })} />)}
        </>
      )}
      {st.step === 4 && (
        <>
          <StepHeading title="How much do you owe on your mortgage?" subtitle="An estimate is fine." />
          {MORTGAGE_RANGES.map(opt => <ChoiceCard key={opt} label={opt} selected={st.mortgageBalance === opt} onPress={() => autoAdvance({ mortgageBalance:opt })} />)}
        </>
      )}
      {st.step === 5 && (
        <>
          <StepHeading title="How much cash are you looking to pull out?" subtitle="An estimate is fine." />
          {CASH_RANGES.map(opt => <ChoiceCard key={opt} label={opt} selected={st.cashNeeded === opt} onPress={() => autoAdvance({ cashNeeded:opt })} />)}
        </>
      )}
      {st.step === 6 && (
        <>
          <StepHeading title="What will you use the cash for?" subtitle="Select all that apply." />
          {CASH_PURPOSES.map(opt => <MultiChoiceCard key={opt} label={opt} selected={st.cashPurposes.includes(opt)} onPress={() => togglePurpose(opt)} />)}
          <FunnelButton label="Continue" onPress={advance} disabled={st.cashPurposes.length === 0} testID="btn-continue" />
        </>
      )}
      {st.step === 7 && (
        <>
      ...
      {st.step === 9 && (
        <>
          ...
          <FunnelButton label={isSubmitting ? "Submitting..." : "Get My Cash-Out Options"} onPress={handleSubmit} disabled={isSubmitting || !st.email || !st.phone} variant="accent" testID="btn-submit" />
        </>
      )}
    </FunnelScreen>
  );
