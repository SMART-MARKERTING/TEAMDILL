...
const RATE_RANGES = ["Under 4%","4% – 5%","5% – 6%","6% – 7%","7% – 8%","Over 8%","Not sure"];
...
type FS = {
  ...
  homeValue: string; mortgageBalance: string; currentRate: string; primaryGoal: string; creditScore: string;
  dobMonth: string; dobDay: string; dobYear: string;
  email: string; phone: string;
};
const DEFAULT: FS = { step:1, firstName:"", lastName:"", address:"", city:"", stateCode:"", zip:"", homeValue:"", mortgageBalance:"", currentRate:"", primaryGoal:"", creditScore:"", dobMonth:"", dobDay:"", dobYear:"", email:"", phone:"" };
...
export default function RateReductionFunnel() {
  ...
  return (
    <FunnelScreen step={st.step} totalSteps={TOTAL} onBack={back} label="RATE REDUCTION">
      {st.step === 1 && (
        <>
      ...
      {st.step === 5 && (
        <>
          <StepHeading title="What's your current interest rate?" subtitle="An estimate is fine if you're not sure." />
          {RATE_RANGES.map(opt => <ChoiceCard key={opt} label={opt} selected={st.currentRate === opt} onPress={() => autoAdvance({ currentRate:opt })} />)}
        </>
      )}
      {st.step === 6 && (
        <>
          <StepHeading title="What's your primary goal?" subtitle="Pick the one that matters most to you." />
          {GOALS.map(opt => <ChoiceCard key={opt} label={opt} selected={st.primaryGoal === opt} onPress={() => autoAdvance({ primaryGoal:opt })} />)}
        </>
      )}
      {st.step === 7 && (
        <>
      ...
      {st.step === 9 && (
        <>
          ...
          <FunnelButton label={isSubmitting ? "Submitting..." : "See My Refinance Options"} onPress={handleSubmit} disabled={isSubmitting || !st.email || !st.phone} variant="accent" testID="btn-submit" />
        </>
      )}
    </FunnelScreen>
  );
